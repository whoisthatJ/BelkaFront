...
namespace UnityEngine.UI.Extensions
{
    ...
    public class ScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollSnap
    {
        ...
        [SerializeField] private Transform _containerPages;
        [SerializeField] private GameObject _prefabPage;
        private List<Image> _pagesUI = new List<Image>();
        private int _currentPageUI;
        private Vector2 _startSizePageUI;
        ...
        public Color SelectDot;
        public Color UnSelectDot;
        ...
        IEnumerator Start()
        {
            ...
            if (direction == ScrollDirection.Horizontal){
                _group = _scroll_rect.content.GetComponent<HorizontalLayoutGroup>();
            }
            else
            {
                _group = _scroll_rect.content.GetComponent<VerticalLayoutGroup>();
            }
            ...
            if (Application.isPlaying)
            {
                ...
                {
                    GameObject page = Instantiate(_prefabPage.gameObject, _containerPages);
                    page.gameObject.SetActive(true);
                    _pagesUI.Add(page.GetComponent<Image>());
                }
                
                var rect = _pagesUI[0].GetComponent<RectTransform>();
                _startSizePageUI = rect.sizeDelta;
                rect.sizeDelta = new Vector2(rect.sizeDelta.x * 1.5f, rect.sizeDelta.y * 1.5f);
                var image = _pagesUI[0].GetComponent<Image>();
                image.color = SelectDot;
                _currentPageUI = 0;
            }
        }
        ...
        public void UpdateListItemsSize()
        {
            ...
            float spacing = 0;
            if (Application.isPlaying) spacing = _group.spacing;
            
            _itemSize = size + spacing;
        ...
        void LateUpdate()
        {
            ...
            if (_lerp)
            {
                ...
                _listContainerTransform.localPosition = Vector3.Lerp(_listContainerTransform.localPosition, _lerpTarget, 15f * Time.deltaTime);

                if (Vector3.Distance(_listContainerTransform.localPosition, _lerpTarget) < 1f)
                {
        ...
        public void UpdatePageUI(float page)
        {
            if (_pagesUI.Count == 0) return;

            _currentPageUI = Mathf.RoundToInt(page);
            
            foreach (var image in _pagesUI)
            {
                var rect = image.GetComponent<RectTransform>(); 
                rect.sizeDelta = new Vector2(_startSizePageUI.x, _startSizePageUI.y);
                image.color = UnSelectDot;
            }
            
            var r = _pagesUI[_currentPageUI].GetComponent<RectTransform>();
            r.sizeDelta = new Vector2(r.sizeDelta.x * 1.5f, r.sizeDelta.y * 1.5f);
            _pagesUI[_currentPageUI].color = SelectDot;
        }
